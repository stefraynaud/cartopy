...
class Gridliner(object):
    ...
    def __init__(self, axes, crs, draw_labels=False, xlocator=None,
        ...
        #: The inline labels which were created at draw time.
        self.inline_label_artists = []
    ...
    def _draw_gridliner(self, nx=None, ny=None, renderer=None):
        ...
        self.inline_label_artists.clear()
                ...
                if line.intersects(map_boundary):
                    ...
                    for tail, head in zip(tails, heads):
                        for i, (pt0, pt1) in enumerate([tail, head]):
                            kw, orig_angle, loc = self._segment_to_text_specs(
                                pt0, pt1, lonlat)
                            ...
                            if x_inline or y_inline:
                                ...
                                if self.rotate_labels:
                                    for abs_delta_angle in np.arange(
                                            delta_angle,
                                            max_delta_angle+1,
                                            delta_angle):
                                        angles.append(artist._angle +
                                                      abs_delta_angle)
                                        angles.append(artist._angle -
                                                      abs_delta_angle)
                            ...
                            for angle in angles:
                                ...
                                if ('3.1.0' <= matplotlib.__version__
                                        <= '3.1.2'):
                                ...
                                if new_loc and loc != new_loc:
                                    loc = new_loc
                                    artist.update(
                                        self._get_alignments_from_loc(loc))
                            ...
                            # Updates
                            if loc == 'inline':
                                self.inline_label_artists.append(artist)
                            ...
                            xy_labels = getattr(self, xy+'label_artists')
